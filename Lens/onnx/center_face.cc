...
static const int64_t INPUT_TENSOR_SHAPE[INPUT_TENSOR_RANK] = { 1, EXPECTED_ROWS, EXPECTED_COLS, EXPECTED_CHANNELS };
...
std::unique_ptr<center_face> center_face::build(const std::string& path)
{
    ...
    return std::unique_ptr<center_face>(new center_face_impl(new Ort::Session(env, path.c_str(), session_options)));
}
