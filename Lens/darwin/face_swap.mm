...
- (instancetype)initWithCompositor:(id<MTLLibrary>)compositor device:(id<MTLDevice>)device queue:(oneapi::tbb::concurrent_bounded_queue<compositor_face2face>*)queue;
...
face_swap_impl::face_swap_impl(const std::vector<MLModel const *>& model_pool,
        ...
        compositor_queue(),
        compositor_thread([[CompositorThread alloc] initWithCompositor:compositor device:device queue:&compositor_queue])
{
...
face2face* face_swap_impl::run(cv::Mat &in_face)
{
    ...
    @autoreleasepool
    {
        ...
        cv::Mat(224, 224, CV_32FC1, [out_celebrity_face_mask_data dataPointer]).copyTo(result->mask);
    }
...
- (instancetype)initWithCompositor:compositor device:device queue:(oneapi::tbb::concurrent_bounded_queue<compositor_face2face>*)queue {
    [super init];
    ...
    [_compositor_pipeline_descriptor setVertexFunction:[compositor newFunctionWithName:@"vertex_main"]];
    [_compositor_pipeline_descriptor setFragmentFunction:[compositor newFunctionWithName:@"fragment_main"]];
    [[_compositor_pipeline_descriptor colorAttachments][0] setPixelFormat:MTLPixelFormatRGBA8Unorm];

    _compositor_state = [device newRenderPipelineStateWithDescriptor:_compositor_pipeline_descriptor error:nil];
...
- (void)composite:(compositor_face2face *)wrapper {
    ...
    frame_descriptor.usage = MTLTextureUsageShaderRead | MTLTextureUsageShaderWrite | MTLTextureUsageRenderTarget;
    plane_descriptor.usage = MTLTextureUsageShaderRead | MTLTextureUsageShaderWrite | MTLTextureUsageRenderTarget;
    ...
    @autoreleasepool
    {
        ...
        id<MTLBuffer> vertex_buffer = [_device newBufferWithBytes:vertices
        ...
        id<MTLSamplerState> sampler_state = [_device newSamplerStateWithDescriptor:sampler_descriptor];
