    ...
    func nextFrame() -> UnsafeMutableRawPointer? {
        ...
        for row in 0..<cellRows {
            for column in 0..<cellColumns {
                ...
                let dotDiameter = 3.0 * Double(cellDiameter) / 4.0 -
                            1.0 * dotDiameterLerpClamped * Double(cellDiameter) / 4.0 // b/w 1/2 and 3/4th of cell diameter
                let dotPadding = (Double(cellDiameter) - dotDiameter) / Double(2)

                context.fillEllipse(in: CGRect(x: x + dotPadding,
                                                y: y + dotPadding,
                                                width: dotDiameter,
                                                height: dotDiameter))
            }
