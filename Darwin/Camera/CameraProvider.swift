...
class CameraProviderSource: NSObject, CMIOExtensionProviderSource {
        ...
        -> CMIOExtensionProviderProperties
    {
        ...
        if properties.contains(magicProperty) {
            providerProperties.setPropertyState(
        ...
        if let state = providerProperties.propertiesDictionary[stateProperty] {
            if let stateData = state.value as? NSData {
                ...
                do {
                    importFromXML(
                        try XMLDocument(
                            data: stateData.subdata(
                                with: NSRange(location: 0, length: stateData.length - 1))
                                as Data))
                } catch {
    ...
    func importFromXML(_ document: XMLDocument) {
        ...
        guard apiVersions.count == 1 else {
            logger.warning(
                "importFromXML: <apiVersion> declaration is not present or is duplicated")
            return
        }
        ...
        guard apiVersion == "v1" else {
            logger.warning(
                "importFromXML: \(apiVersion) specified in <apiVersion> is not supported")
            return
        }
        ...
        for (videoIndex, video) in videos.enumerated() {
            ...
            guard
                id.count <= 1 && name.count <= 1 && width.count <= 1 && height.count <= 1
                    && frameRate.count <= 1
            else {
                logger.warning("importFromXML: Malformed <device> at index \(videoIndex)")
                continue
            }
            guard name.count == 1 && width.count == 1 && height.count == 1 && frameRate.count == 1
            else {
                logger.warning(
                    "importFromXML: Missing name, width, height, or frameRate at <video> \(videoIndex)"
                )
                continue
            }
            ...
            guard let nameValue = name[0].stringValue,
                ...
                let frameRateValue = UInt32(frameRate[0].stringValue ?? "")
            else {
            ...
            guard widthValue >= 16 && widthValue <= 8192 else {
                logger.warning(
                    "importFromXML: <width> out of bounds [16ox, 8192px] at <video> \(videoIndex)")
                continue
            }
            guard heightValue >= 16 && heightValue <= 8192 else {
                logger.warning(
                    "importFromXML: <height> out of bounds [16px, 8192px] at <video> \(videoIndex)")
                continue
            }
            guard frameRateValue >= 10 && frameRateValue <= 120 else {
                logger.warning(
                    "importFromXML: <frameRate> out of bounds [10FPS, 120FPS] at <video> \(videoIndex)"
                )
                continue
            }
            ...
            if let idValue = idValue {
                guard
                    let modifiedDevice = deviceSources.first(where: {
                ...
                if widthValue != modifiedDevice.width || heightValue != modifiedDevice.height
                    || frameRateValue != modifiedDevice.frameRate
                {
                    modifiedDevice.setDeviceFormat(
                        width: widthValue,
                        height: heightValue,
